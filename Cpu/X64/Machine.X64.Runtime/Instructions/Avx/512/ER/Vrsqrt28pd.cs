...
{
    private void vrsqrt28pd(in Instruction instruction)
    {
        switch (instruction.Code)
        {
            case Code.EVEX_Vrsqrt28pd_zmm_k1z_zmmm512b64_sae:
                {
                    ...
                    for (int i = 0; i < Vector512<double>.Count; i++)
                        result = result.WithElement(i, Math.ReciprocalSqrtEstimate(v512[i]));
