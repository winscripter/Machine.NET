...
{
    private void vmaxps(in Instruction instruction)
    {
        switch (instruction.Code)
        {
            case Code.EVEX_Vmaxps_xmm_k1z_xmm_xmmm128b32:
                {
                    ...
                    Vector128<float> result = VectorMinMax.OfMax(parameter1, parameter2);
                    this.ProcessorRegisters.SetXmm(instruction.GetOpRegister(0), result);
                    break;
                }

            case Code.EVEX_Vmaxps_ymm_k1z_ymm_ymmm256b32:
                {
                    ...
                    Vector256<float> result = VectorMinMax.OfMax(parameter1, parameter2);
                    this.ProcessorRegisters.SetYmm(instruction.GetOpRegister(0), result);
                    break;
                }

            case Code.EVEX_Vmaxps_zmm_k1z_zmm_zmmm512b32_sae:
                {
                    ...
                    Vector512<float> result = VectorMinMax.OfMax(parameter1, parameter2);
