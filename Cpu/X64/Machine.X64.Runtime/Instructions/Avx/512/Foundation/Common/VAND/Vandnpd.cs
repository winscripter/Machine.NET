...
{
    private void vandnpd(in Instruction instruction)
    {
        switch (instruction.Code)
        {
            case Code.EVEX_Vandnpd_xmm_k1z_xmm_xmmm128b64:
                {
                    ...
                    for (int i = 0; i < Vector128<double>.Count; i++)
                    {
                        if (!HasBitSetInK1(i))
                        {
                            result = result.WithElement(i, a[i].Andn(b[i]));
                        }
                    }
            ...
            case Code.EVEX_Vandnpd_ymm_k1z_ymm_ymmm256b64:
                {
            ...
            case Code.EVEX_Vandnpd_zmm_k1z_zmm_zmmm512b64:
                {
