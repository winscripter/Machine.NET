    ...
    private void mov(in Instruction instruction)
    {
        switch (instruction.Code)
        {
            case Code.Mov_AL_moffs8:
                {
                    this.ProcessorRegisters.Al = this.Memory[GetMemOperand8(instruction)];
                    break;
                }

            case Code.Mov_AX_moffs16:
                {
                    this.ProcessorRegisters.Ax = this.Memory.ReadUInt16(GetMemOperand16(instruction));
                    break;
                }
            ...
            case Code.Mov_EAX_moffs32:
                {
                    this.ProcessorRegisters.Eax = this.Memory.ReadUInt32(GetMemOperand64(instruction));
                    break;
                }

            case Code.Mov_moffs16_AX:
                {
                    this.Memory.WriteUInt16(GetMemOperand16(instruction), this.ProcessorRegisters.Ax);
                    break;
                }

            case Code.Mov_moffs32_EAX:
                {
                    this.Memory.WriteUInt32(GetMemOperand32(instruction), this.ProcessorRegisters.Eax);
                    break;
                }

            case Code.Mov_moffs64_RAX:
                {
                    this.Memory.WriteUInt64(GetMemOperand64(instruction), this.ProcessorRegisters.Rax);
                    break;
                }

            case Code.Mov_moffs8_AL:
                {
                    this.Memory[GetMemOperand64(instruction)] = this.ProcessorRegisters.Al;
                    break;
                }
            ...
            case Code.Mov_r32m16_Sreg:
                {
                    ...
                    {
                        this.Memory.WriteUInt16(GetMemOperand64(instruction), (ushort)this.ProcessorRegisters.EvaluateSReg(instruction.GetOpRegister(1)));
                    }
                    break;
                }
            ...
            case Code.Mov_RAX_moffs64:
                {
                    this.ProcessorRegisters.Rax = GetMemOperand64(instruction);
                    break;
                }
