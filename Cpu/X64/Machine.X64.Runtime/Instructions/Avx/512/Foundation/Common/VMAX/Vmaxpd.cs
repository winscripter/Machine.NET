...
{
    private void vmaxpd(in Instruction instruction)
    {
        switch (instruction.Code)
        {
            case Code.EVEX_Vmaxpd_xmm_k1z_xmm_xmmm128b64:
                {
                    ...
                    Vector128<double> result = VectorMinMax.OfMax(parameter1, parameter2);
                    this.ProcessorRegisters.SetXmm(instruction.GetOpRegister(0), result.As<double, float>());
                    break;
                }

            case Code.EVEX_Vmaxpd_ymm_k1z_ymm_ymmm256b64:
                {
                    ...
                    Vector256<double> result = VectorMinMax.OfMax(parameter1, parameter2);
                    this.ProcessorRegisters.SetYmm(instruction.GetOpRegister(0), result.As<double, float>());
                    break;
                }

            case Code.EVEX_Vmaxpd_zmm_k1z_zmm_zmmm512b64_sae:
                {
                    ...
                    Vector512<double> result = VectorMinMax.OfMax(parameter1, parameter2);
