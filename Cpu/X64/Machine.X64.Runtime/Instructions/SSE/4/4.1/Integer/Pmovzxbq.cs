...
{
    private void pmovzxbq(in Instruction instruction)
    {
        switch (instruction.Code)
        {
            case Code.Pmovzxbq_xmm_xmmm16:
                {
                    Vector64<sbyte> src = instruction.GetOpKind(1) switch
                    {
                        OpKind.Memory => this.Memory.ReadBinaryVector16<sbyte>(GetMemOperand(in instruction)),
